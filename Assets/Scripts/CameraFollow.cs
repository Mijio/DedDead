...
public class CameraFollow : MonoBehaviour
{
 ...
 [SerializeField] Vector3 velocity = Vector3.zero;
    ...
    [ReadOnly]
#endif
 [SerializeField] float vertExtent;
    #if UNITY_EDITOR
    [ReadOnly]
#endif
 [SerializeField] float horzExtent;
    #if UNITY_EDITOR
    [ReadOnly]
#endif
 [SerializeField] Vector3 tempVec = Vector3.one;
    #if UNITY_EDITOR
    [ReadOnly]
#endif
 [SerializeField] bool isBoundHorizontal;
    #if UNITY_EDITOR
    [ReadOnly]
#endif
 [SerializeField] bool isBoundVertical;
    #if UNITY_EDITOR
    [ReadOnly]
#endif
 [SerializeField] bool isFollowHorizontal;
    #if UNITY_EDITOR
    [ReadOnly]
#endif
 [SerializeField] bool isFollowVertical;
    #if UNITY_EDITOR
    [ReadOnly]
#endif
 [SerializeField] bool isDeadZoneHorizontal;
    #if UNITY_EDITOR
    [ReadOnly]
#endif
 [SerializeField] bool isDeadZoneVertical;
    #if UNITY_EDITOR
    [ReadOnly]
#endif
 [SerializeField] Vector3 deltaCenterVec;
    ...
    private void OnDrawGizmos()
    {
        ...
        Gizmos.DrawWireCube(new Vector3((rightBound + leftBound) / 2, (upperBound + lowerBound) / 2),
            new Vector2(rightBound - leftBound, upperBound - lowerBound));
        if (target != null)
        {
            Gizmos.color = Color.magenta;
            Gizmos.DrawWireCube(target.position,
                new Vector2(rightDeadBound + leftDeadBound, lowerDeadBound + upperDeadBound));
        }
    }
    ...
    private void OnValidate()
    {
        ...
        if (leftDeadBound > rightDeadBound)
        {
            leftDeadBound = rightDeadBound;
        }
        if (lowerDeadBound > upperDeadBound)
        {
            lowerDeadBound = upperDeadBound;
        }
